...
Publishing data in an Open data portal means creating both a dataset and its resource(s) containing the actual data or a link pointing to it (details of the format available above within the [Open data format](#the-open-data-format) part).  
But it is also possible to create a dataset alone and to add resources later.
...
If a dataset with the same title already exists, it will either create a resource if there is no resource within the dataset with the same title, or modify the existing resource identified by this title.
In the end, the only difference between a creation and a modification of dataset or resource is defined by the existence (or not) of a dataset or resource with the same title.

MISP gives then a confirmation message with the link of the newly created dataset or resource:
```
...
Note that if only a dataset title is provided, the portal will be queried for a full dataset delete, which includes all its resources.  
Alternatively, if at least 1 resource title is provided, it will only delete the mentioned resources, and the dataset will still exist, with its potential resources that have not been deleted.
...
List of non exhaustive possible improvement for the implementation of the Open data feature:
- Make it available for other Open data portals (european national portals are currently tested to be supported, like the French one). Status of the currently supported portals available [here](https://github.com/misp/misp-opendata/portals_support.md).
